...
if __name__ == "__main__":
    ...
    regressor = build_GRU_model(indicators, timesteps, X_train1, y_train1,
                                 units=256, dropout_rate=0.3, epochs=75, batch_size=32)
    regressor = Incremental_Learning_GRU_model(regressor, X_train2, y_train2, new_epochs=75, new_batch_size=32)
    regressor = Incremental_Learning_GRU_model(regressor, X_train3, y_train3, new_epochs=75, new_batch_size=32)
    ...
    file_path1 = os.path.join('E:\Option_data\上证50ETF\实证选择数据\LSTM_(20231227,2.608,20160)',
                              f"(20231227,2.608,20160)_测试集_GRU训练后.csv")
    dataset_test.to_csv(file_path1, index=False)
    # 保存模型
    model_path = r'E:\Option_data\上证50ETF\实证选择数据\LSTM_(20231227,2.608,20160)\gru_sz50_model.h5'
