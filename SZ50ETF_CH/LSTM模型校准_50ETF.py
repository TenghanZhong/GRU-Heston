...
import pandas as pd
from sklearn.preprocessing import MinMaxScaler#sklearn.preprocessing  minmaxscaler
from keras.models import Sequential#kearas.models
...
def preprocess_data(dataset_train,timesteps,objective):
    ...
    train_set_scaled = sc.fit_transform(train_set)#sc=minmaxscaler(feature_range=(0,1))
    ...
    for i in range(timesteps, len(train_set_scaled)):
        ...
        # #注意X_train为三维数据，append所增加的是行维度，原来的行维度变成列维度，原来的列维度变为z维度
...
def build_regressor(units=128,dropout_rate=0.2):
    ...
    regressor.add(Dense(units=1))#输出只有一个值
