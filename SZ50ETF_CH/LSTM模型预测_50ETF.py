...
if __name__ == "__main__":
    dataset_train1 = pd.read_csv(r'E:\Option_data\上证50ETF\实证选择数据\LSTM_(20231227,2.608,20160)\(20231227,2.608,20160)_8月训练集.csv')
    dataset_train2 = pd.read_csv(
        r'E:\Option_data\上证50ETF\实证选择数据\LSTM_(20231227,2.608,20160)\(20231227,2.608,20160)_9月训练集.csv')
    dataset_train3 = pd.read_csv(
        r'E:\Option_data\上证50ETF\实证选择数据\LSTM_(20231227,2.608,20160)\(20231227,2.608,20160)_10月训练集.csv')
    dataset_test = pd.read_csv(r'E:\Option_data\上证50ETF\实证选择数据\LSTM_(20231227,2.608,20160)\(20231227,2.608,20160)_测试集.csv')
    ...
    regressor=build_lstm_model(indicators,timesteps,X_train1,y_train1,
                               units=256, dropout_rate=0.4,epochs=75,batch_size=32)
    regressor = Incremental_Learning_lstm_model(regressor, X_train2, y_train2,new_epochs=75,new_batch_size=32)
    regressor = Incremental_Learning_lstm_model(regressor, X_train3, y_train3, new_epochs=75, new_batch_size=32)
    ...
    '''
    #保存训练后的测试集文件
    dataset_test['lstm_heston_price']=dataset_test['heston_price']+dataset_test['predicted_error']
    file_path1 = os.path.join('E:\Option_data\上证50ETF\实证选择数据\LSTM_(20231227,2.608,20160)', f"(20231227,2.608,20160)_测试集_LSTM训练后.csv")
    dataset_test.to_csv(file_path1, index=False)

    # 保存模型
    model_path = r'E:\Option_data\上证50ETF\实证选择数据\LSTM_(20231227,2.608,20160)\lstm_sz50_model.h5'
