...
if __name__ == "__main__":
    dataset_train1 = pd.read_csv(
        r'E:\Option_data\上证50ETF\实证选择数据\LSTM_(20231227,2.608,20160)\(20231227,2.608,20160)_8月训练集.csv')
    ...
    dataset_test = pd.read_csv(
        r'E:\Option_data\上证50ETF\实证选择数据\LSTM_(20231227,2.608,20160)\(20231227,2.608,20160)_测试集.csv')

    # 设置训练目标和步长
    objective = ('c','ttm_ratio','s')  # #要预测的值放在第一个,其他特征放后面 (构建模型只预测第一个值，Dense(units=1))
    timesteps = 60  # 预测步长
    ...
    regressor = build_GRU_model(indicators, timesteps, X_train1, y_train1,
                                 units=256, dropout_rate=0.4, epochs=50, batch_size=32)
    regressor = Incremental_Learning_GRU_model(regressor, X_train2, y_train2, new_epochs=50, new_batch_size=32)
    regressor = Incremental_Learning_GRU_model(regressor, X_train3, y_train3, new_epochs=50, new_batch_size=32)
    ...
    # 保存训练后的测试集文件
    dataset_test['gru_price'] = dataset_test['predicted_price']
    file_path1 = os.path.join('E:\Option_data\上证50ETF\实证选择数据\LSTM_(20231227,2.608,20160)',
                              f"(20231227,2.608,20160)_GRU直接预测测试集.csv")
    dataset_test.to_csv(file_path1, index=False)
    # 保存模型
    model_path = r'E:\Option_data\上证50ETF\实证选择数据\LSTM_(20231227,2.608,20160)\gru_price_model.h5'
