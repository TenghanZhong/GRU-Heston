...
def calculate_single_Heston_value(df,params):
    ...
    rf = df['rf']  # 无风险利率
    dividend = 0#df['dividend']  # 股息率,在最后应用模型的时候考虑股息拟合会更好
...
def calculate_all_Heston_value(df,params):
    ...
    #pool.starmap(calculate_single_Heston_value,[(df.iloc[i],params) for i in range(len(df))])
...
if __name__ == "__main__":
    ...
    # theta, kappa, sigma, rho, v0 = params
    calibrated_params=(0.335467 , 0.773346 , 0.72024 , 0.620928 , 0.001028)
    # 8月  0807-0808：  最优参数：(0.001403, 17.366490, 0.220803, -0.999698, 0.183595)#Square Error (%) : 0.003106942
    #9月 0904-0905 ：  最优参数： (0.580888 , 0.360369 , 0.596251 , 0.269848,  0.001174) Mean Square Error (%) : 0.000815823
    #10月 1009-1010 ：  最优参数：(0.335467 , 0.773346 , 0.72024 , 0.620928 , 0.001028) Mean Square Error (%) : 0.000024255
    ...
    month=10
    file_path1 = os.path.join('E:\Option_data\上证50ETF\实证选择数据\LSTM_(20231227,2.608,20160)', f"(20231227,2.608,20160)_{month}月训练集.csv")
    ...
    data_test = data_test.reset_index(drop=True)#drop=True 参数表示不将旧的索引作为新的列保留。
    ...
    '''
    file_path2 = os.path.join('E:\Option_data\上证50ETF\实证选择数据\LSTM_(20231227,2.608,20160)',
                              f"(20231227,2.608,20160)_测试集.csv")
