...
def plot_price_comparison_chart(data):
    ...
    plt.savefig(f'E:\Option_data\上证50ETF\实证选择数据\拟合图像\BSMODEL_完整期({month_day1}-{month_day2}).png')
    plt.show()
...
if __name__ == "__main__":
    ...
    data['maturity_date'], data['date'] = (pd.to_datetime(data['maturity_date'].astype(str)).
                                                     ...
                                                     pd.to_datetime(data['date'].astype(str)).dt.strftime(
                                                         '%Y%m%d'))
    #implied_vol=data['implied_volatility'].mean()  #=0.1748207059869053
    ...
    file_path2 = os.path.join('E:\Option_data\上证50ETF\实证选择数据\LSTM_(20231227,2.608,20160)',
                              f"(20231227,2.608,20160)_测试集_bsm.csv")
