...
def cost_function_generator_multiprocessing(option_data, norm=True):
    def cost_function(params):
        ...
        if norm:
            total_error = np.sqrt(np.sum(error))  ##加权均方误差
...
def get_heston_param_TRR(data,initial_params,bounds):
    ...
    # 差分进化
    sol = optimize.differential_evolution(cost_function, bounds=bounds,constraints=[nlc]) #callback=callback)
...
def plot_price_comparison_chart(data,params):
    ...
    plt.savefig(rf'E:\Option_data\沪深300ETF\300etf拟合图像\{params[0]}_校准期({month_day1}-{month_day2}).png')
    plt.show()
...
if __name__ == "__main__":
    ...
    data['maturity_date'], data['date'] = (pd.to_datetime(data['maturity_date'].astype(str)).
                                                     ...
                                                     pd.to_datetime(data['date'].astype(str)).dt.strftime(
                                                         '%Y%m%d'))
    data = data.query( "'20230807' <= date <= '20230808'")#选择参数估计的日子 0807-0808  0904-0905 1009-1010
    data = data.sort_values(by=['datetime'])
    data = data.reset_index(drop=True) #必须做，在cost_funtion_generation当中会用到
    ...
    with open(rf'E:\Option_data\沪深300ETF\300etf拟合图像\error_summary({month_day1}-{month_day2}).txt', 'w') as file:
