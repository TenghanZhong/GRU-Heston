...
if __name__ == "__main__":
    ...
    parameters_dictionary = {'batch_size': [64,32],
                             'epochs': [75,100],
                             'optimizer': ['adam','rmsprop'],
                             'units': [256],
                             'dropout_rate': [0.3]}  # 要检验的值，经验主义 ,rmsprop一般是常用的随机梯度下降方法
    grid_search = GridSearchCV(estimator=regressor,
                               param_grid=parameters_dictionary,
                               scoring='neg_mean_squared_error',
                               cv=None)
    grid_search = grid_search.fit(X_train, y_train)
    #当前数据集最优参数: 全体：{'batch_size': 64, 'dropout_rate': 0.4, 'epochs': 75, 'optimizer': 'adam', 'units': 256}
    #分别：{'batch_size': 64, 'dropout_rate': 0.3, 'epochs': 100, 'optimizer': 'adam', 'units': 256}
