...
if __name__ == "__main__":
    ...
    dataset_all = pd.concat([dataset_train1, dataset_train2], axis=0)

    # 分阶段优化
    objective = ('c', 's', 'ttm_ratio')  # 要预测的值
    timesteps = 60  # 预测步长
    datasets = [dataset_train1, dataset_train2, dataset_train3]
    best_params = None
    combo_count=0

    X_train, y_train, indicators, sc = preprocess_data(dataset_train1, timesteps, objective)
    regressor = KerasRegressor(build_fn=build_regressor, units=256, dropout_rate=0.3)
    parameters_dictionary = {'batch_size': [16,32,64],
                             'epochs': [50,75,100],
                             'optimizer': ['adam'],
                             'units': [256,512],
