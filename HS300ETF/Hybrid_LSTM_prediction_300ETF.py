...
if __name__ == "__main__":
    ...
    regressor = Incremental_Learning_lstm_model(regressor, X_train2, y_train2, new_epochs=75, new_batch_size=64)
    regressor = Incremental_Learning_lstm_model(regressor, X_train3, y_train3, new_epochs=75, new_batch_size=64)
    ...
    file_path1 = os.path.join(r'E:\Option_data\沪深300ETF\300ETF实证选择数据\LSTM_(20231227, 3.6, 20160)', f"(20231227, 3.6, 20160)_测试集_LSTM训练后.csv")
    dataset_test.to_csv(file_path1, index=False)
    # 保存模型
    model_path = r'E:\Option_data\沪深300ETF\300ETF实证选择数据\LSTM_(20231227, 3.6, 20160)\lstm_hs300_model.h5'
